...
export async function submitScore(
  ...
  if (mode !== 'classic' && mode !== 'time_attack' && mode !== 'obstacles') throw new Error('Invalid mode')
  const payload: TablesInsert<'scores'> = {
    user_id: user.id,
    score,
    mode,
...
export async function getGlobalLeaderboard(params?: { limit?: number; offset?: number }) {
  ...
  if (error) throw error
  return data as Tables<'leaderboard_global'>[]
}
...
export async function getModeLeaderboard(mode: GameMode, params?: { limit?: number; offset?: number }) {
  ...
  const { data, error } = await client
    ...
    .range(offset, offset + limit - 1)
  if (error) throw error
  return data as Tables<'leaderboard'>[]
}
